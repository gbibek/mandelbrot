...
std::vector<each_pixel> do_the_pixel(double x, double y){
    ...
    while(i<number_of_time){
	    ...
	    value = mandtest(dummy_x,dummy_y);
            ...
            _each_pixel.x_axis = x;
            _each_pixel.y_axis = y;
...

{
    ...
    std::vector<hpx::future<std::vector<each_pixel>>> vector_future;  //vector which holds futures that will later call do_the_pixel
        ...
        for(int i=0; i < counter;i++) {
            ...
            for( y_ = final_y ; y_ < (final_y + number_of_time);y_++){
                ...
                for( x_ = final_x; x_ < (final_x+ number_of_time);x_++){    
                    ...
                    if(future_return[i_].mantest_no ==maxiteration){
